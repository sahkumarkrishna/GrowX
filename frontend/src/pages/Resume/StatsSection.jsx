...
export default function StatsSection() {
  const stats = [
    { icon: Users, value: '1,000+', label: 'Job Seekers', color: 'from-blue-500 to-cyan-500' },
    { icon: FileText, value: '10+', label: 'Templates', color: 'from-purple-500 to-pink-500' },
    { icon: Award, value: '500+', label: 'Resumes Created', color: 'from-orange-500 to-red-500' },
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          ...
          <p className="text-xl text-blue-200">
            Join thousands creating professional resumes every day
          </p>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-16 text-center">
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-lg px-6 py-3 rounded-full border border-white/20">
            ...
            <span className="font-semibold">4.7/5 average rating from users</span>
          </div>
        </motion.div>
      </div>
    </section>
