...
const JOB_NAV = [
  ...
  { path: '/joall',  label: 'All Jobs',   icon: Building2, sub: 'View every listing'    },
...
// ── Mobile profile dropdown (avatar → dropdown with Dashboard) ────────────────
function MobileProfileMenu({ user, onLogout }) {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1.5 rounded-xl hover:bg-gray-100
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 8, scale: 0.97 }}
            transition={{ duration: 0.15 }}
            className="absolute top-full right-0 mt-2 w-56 bg-white rounded-2xl
                       shadow-xl border border-gray-100 overflow-hidden z-50">
    ...
    <>
      <nav className="bg-white/95 backdrop-blur-xl sticky top-0 z-50
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16
          ...
          {/* ── Desktop right: Login/Signup OR Menu icon only ────────── */}
          <div className="hidden lg:flex items-center gap-3 shrink-0">
            {!user ? (
              ...
              /* Desktop: ONLY a hamburger Menu icon — no profile */
              <button
                onClick={() => setOpen(o => !o)}
          ...
          {/* ── Mobile: profile avatar (with dropdown) + hamburger ───── */}
      ...
      {/* ── Drawer (used by both desktop Menu icon + mobile hamburger) ── */}
      <AnimatePresence>
        {open && (
          <>
            <motion.div
              ...
              className="fixed inset-0 bg-black/30 backdrop-blur-sm z-40 top-16" />
            ...
            <motion.div
              ...
              className="fixed inset-x-0 top-16 z-50 bg-white border-b border-gray-100
                ...
                {user && (
                  <>
                      ...
                      <>
                        <Link to="/user/dashboard" onClick={() => setOpen(false)}
                          className="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm
                                     font-semibold text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-colors">
                          <div className="w-8 h-8 rounded-xl bg-blue-100 flex items-center justify-center shrink-0">
                        ...
                        <Link to="/user/profile" onClick={() => setOpen(false)}
                          className="flex items-center gap-3 px-4 py-3 rounded-2xl text-sm
                                     font-semibold text-gray-700 hover:bg-purple-50 hover:text-purple-600 transition-colors">
                          <div className="w-8 h-8 rounded-xl bg-purple-100 flex items-center justify-center shrink-0">
                    ...
                    <button onClick={() => { handleLogout(); setOpen(false); }}
                      className="flex items-center gap-3 w-full px-4 py-3 rounded-2xl text-sm
                                 font-semibold text-gray-700 hover:bg-red-50 hover:text-red-600 transition-colors">
